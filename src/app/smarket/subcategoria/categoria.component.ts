...
import { User, UserProfile, UserWork, UserContacts, UserSocial, UserSettings } from './categoria.model';
...
@Component({
  ...
  providers: [ CategoriaService, MenuService ]
})
export class CategoriaComponent implements OnInit {
  ...
  public getUsers(): void {
    this.categoriaService.getUsers().subscribe( users => 
      this.users = users
    );    
  }

  public addUser(user:User){
    this.categoriaService.addUser(user).subscribe(user => {
      this.getUsers();      
    });
  }

  public updateUser(user:User){
    this.categoriaService.updateUser(user).subscribe(user => {
      this.getUsers();      
    });
  }

  public deleteUser(user:User){
    this.categoriaService.deleteUser(user.id).subscribe(result => 
      this.getUsers()
    );
  }
